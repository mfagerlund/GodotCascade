...
GodotCascade 0.5.0 adapts Godot 4.7 `LineEdit` and `TextEdit` for single-line and multiline input. Neither replaces the native editing engine. The release gate verifies the adapter boundary while platform services remain Godot's responsibility.

| Behavior | Automated evidence | 0.5.0 status |
| --- | --- | --- |
