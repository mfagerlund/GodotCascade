...
Named paths are resolved through a retained dependency index. A path such as `item.status` can therefore update matching controls inside already-materialized non-virtual Repeat rows without rebuilding a Repeat candidate; every row keeps its own `item` scope. Invalidating a collection path still validates collection topology. When an Array contains the same keyed item objects in a new order and the row template has no structural/class condition or nested Repeat, the document moves the existing native rows atomically with zero candidates. New/replaced items, changed keys, conditions, nested collections, virtual windows, and any ambiguous case use the ordinary validated candidate-reconciliation path.
