...
GodotCascade 0.8: experimental GXML/GCSS UI that builds native Godot Controls

## Suggested post

I've published GodotCascade 0.8, an experimental retained-mode UI framework for Godot 4.7. It turns a deliberately focused GXML/GCSS language into ordinary Godot `Control` trees. The current preview includes flex, grid, stack, and semantic-table layout; native form controls; reusable typed components; retained typed collections and fixed-height virtualization; indexed invalidation; explicit bindings; optional typed C# generation; source diagnostics; and Godot/VS Code language tools.
...
- [0.8.0 release](https://github.com/mfagerlund/GodotCascade/releases/tag/v0.8.0)
