...
Versions 0.1.0 through 0.8.0 are tagged and published through the reproducible release gate; 0.8.1 is the review-hardened release candidate. The 0.2 interactive-forms slice adds an adapted native single-line text input, explicit writable bindings, validation state, input-modality-aware focus styling, and a live settings workflow exercised in both native and HTML showcase views. Version 0.3 adds the corresponding native multiline adapter, repeated-item write-back, and owned-container hover backgrounds. Version 0.4 adds optional typed C# generation, semantic tables, native scrolling, and dynamic collection examples. Version 0.5 adds targeted invalidation, broader one-way native state, and exact conditional rendering. Version 0.6 adds reusable typed GXML components, debugger dependency traces, and focused GCSS custom properties with typed arithmetic. Version 0.7 adds advanced focused styles, source-language tooling, typed collections, fixed-height virtualization, representative workload evidence, and desktop CI coverage. Version 0.8 adds retained invalidation indexes, zero-candidate pure keyed reorders, a pinned GTML comparison, and reproducible platform-certification/evaluation tooling. Version 0.8.1 closes the independent Codex/Claude review findings across runtime correctness, diagnostics, tooling, accessibility, packaging, and release evidence.
...
- [ ] Submit the prepared 0.8.1 entry through the authenticated Godot Asset Library form
...
- [x] Run iterative independent Codex and Claude reviews, fix every reproducible substantive finding, and record the [0.8.1 hardening evidence](docs/artifacts/review-hardening-2026-08-08.md)
