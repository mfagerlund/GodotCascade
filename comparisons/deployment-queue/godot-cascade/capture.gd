...
const SCENE_PATH := "res://comparisons/deployment-queue/godot-cascade/deployment_queue.tscn"
...
func _capture() -> void:
	...
	if image == null or image.is_empty():
		push_error("Godot returned an empty deployment queue capture")
		quit(2)
		return
	DirAccess.make_dir_recursive_absolute(output.get_base_dir())
	var error := image.save_png(output)
	if error != OK:
		push_error("Could not save deployment queue capture: %s" % error)
		quit(2)
		return
